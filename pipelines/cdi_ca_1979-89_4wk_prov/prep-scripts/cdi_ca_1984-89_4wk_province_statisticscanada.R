...
tidy_dataset = "cdi_ca_1984-89_4wk_prov_statcan"
digitization = "cdi_ca_1984-89_4wk_prov_statcan"
...
get_sheet_names = function(data) {
  grep("template", unique(data$sheet), value = TRUE, invert = TRUE)
}

behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 4, 46), between(col, 2, 42))
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      (startsWith(stat_name_1, 'Total'   ) & startsWith(stat_name_2, '1984')) ~ 'cum_report_year',
      (startsWith(stat_name_1, 'Total'   ) & startsWith(stat_name_2, '1985')) ~ 'cum_report_year',
      (startsWith(stat_name_1, 'Total'   ) & startsWith(stat_name_2, '1986')) ~ 'cum_report_year',
      (startsWith(stat_name_1, 'Total'   ) & startsWith(stat_name_2, '1987')) ~ 'cum_report_year',
      (startsWith(stat_name_1, 'Total'   ) & startsWith(stat_name_2, '1988')) ~ 'cum_report_year',
      (startsWith(stat_name_1, 'Total'   ) & startsWith(stat_name_2, '1989')) ~ 'cum_report_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1982')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1983')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1984')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1985')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1986')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1987')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1988')) ~ 'cum_prev_year'
    ))

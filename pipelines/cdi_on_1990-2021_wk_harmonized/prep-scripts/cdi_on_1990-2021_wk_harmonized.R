...
# ----------------------------------------
dataset_id = "cdi_on_1990-2021_wk_harmonized"
# ----------------------------------------
...
harmonize_data = function(tidy_cdi, disease_lookup) {
  ...
  (tidy_cdi
   # Removing suspected cases: only keep those with diagnosis_certainty = confirmed
   |> filter(diagnosis_certainty == 'confirmed')
   ...
   |> summarise(cases_this_period = sum(cases_this_period), digitization_id = unique(digitization_id))
   |> ungroup()

...
tidy_dataset = "cdi_ca_1963_wk_prov_dbs"
digitization = "cdi_ca_1963_wk_prov_dbs"
...
clean_sheets = function(
  ...
  legend = data.frame(
      character = c('.'             , '..'           , '. .'          , '**'           , '-')
    , key       = c('Not reportable', 'Not available', 'Not available', 'Not available', 0  )
  )
  ...
  (beheaded_sheet
    ...
    %>% mutate(numeric = ifelse(character %in% legend$character | !is.na(character), key, numeric))
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1963'      ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1962'      ) ~ 'cum_prev_year',

...
tidy_dataset = "cdi_ca_1962_wk_prov_dbs"
digitization = "cdi_ca_1962_wk_prov_dbs"
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1962'      ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1961'      ) ~ 'cum_prev_year',
      startsWith(stat_name_1, 'Median'    ) ~ 'median_prev_5_years',
      startsWith(stat_name_1, 'Cumulative') ~ 'cum_median_prev_5_years'
    ))
...
combine_weeks = function(cleaned_sheets, sheet_dates, metadata) {
  (cleaned_sheets
   ...
   %>% select(location, period_start_date, period_end_date, historical_disease,
              icd_7, historical_disease_subclass, icd_7_subclass,
...
get_periods = function(sheets, data, metadata) {
  (data
   ...
   ## due to footnote at the bottom of page 9 in scan cdi_ca_1962_wk_prov_dbs_p4:
   ## "Pages 2 to 8 of previous week's report should read September 22, 1962"
   |> mutate(end_date = if_else(
        end_date == as.Date("1962-09-28")
      , as.Date("1962-09-22")
      , end_date
   ))

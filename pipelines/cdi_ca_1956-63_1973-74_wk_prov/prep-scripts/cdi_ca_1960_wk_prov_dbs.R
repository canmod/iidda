...
tidy_dataset = "cdi_ca_1960_wk_prov_dbs"
digitization = "cdi_ca_1960_wk_prov_dbs"
...
behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 3, 32), between(col, 2, 56))
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% separate(cases, c('cases', 'legend'), sep = "\\(", extra = "drop", fill = "right") # removes numbers in brackets that are associated with the legend and are not case counts.
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1960'      )
      & any(grepl('1960', beheaded_sheet$stat_name_2)) ~ 'cum_report_year',
      startsWith(stat_name_2, '1959'      )
      & any(grepl('1960', beheaded_sheet$stat_name_2)) ~ 'cum_prev_year',
      startsWith(stat_name_2, '1958'      ) ~ 'cum_prev_year',
      startsWith(stat_name_2, '1959'      )
      & any(grepl('1958', beheaded_sheet$stat_name_2)) ~ 'cum_report_year',

...
# accordingly. There are only merged cells in the median columns, so this script
# properly preps cases_this_period, which is our main concern for now.

# ----------------------------------------
# Information for Locating Metadata
tidy_dataset = "cdi_ca_1973_wk_prov"
digitization = "cdi_ca_1973_wk_prov"
...
behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 3, 33), between(col, 2, 56))
...
clean_sheets = function(
  ...
  subclass_patterns = c("Staph", "Botulism", "Diarr", "Cox",
                        "ECHO", "Not specified")
  family_row = filter(beheaded_sheet, row %in% c(6, 16, 21, 27, 32))
  ...
  (beheaded_sheet
    ...
    %>% mutate(is_subclass = grepl(paste(subclass_patterns, collapse = "|"), disease))
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1973'       ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1972'       ) ~ 'cum_prev_year',
      ...
      (startsWith(stat_name_1, '1972') & startsWith(stat_name_2, 'Median'  )) ~ 'cum_median_prev_5_years'
    ))
    ...
    %>% mutate(disease_family = sub('\\*', '', disease_family))
    ...
    %>% rename(historical_disease = disease,
               historical_disease_subclass = disease_subclass,
               historical_disease_family = disease_family)

    %>% filter(!is_empty(historical_disease))
  )
}
...
combine_weeks = function(cleaned_sheets, sheet_dates, metadata) {
  (cleaned_sheets
   ...
   %>% select(location, period_start_date, period_end_date, historical_disease,
              icd_7, historical_disease_subclass, historical_disease_family, icd_7_subclass,
              cases_this_period, cases_prev_period, cases_cum_report_year,
              cases_cum_prev_year, cases_median_prev_5_years, cases_cum_median_prev_5_years,
   )
   %>% as.data.frame
   %>% identify_scales()

...
tidy_dataset = "cdi_ca_1959_wk_prov_dbs_w1_3"
digitization = "cdi_ca_1959_wk_prov_dbs"
...
get_sheet_names = function(data) {
  grep("^w[0-9]+", unique(data$sheet), value = TRUE)[(1:3)]
}
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_1, '(10) Report'  ) ~ 'this_period',
      startsWith(stat_name_1, 'Previous'     ) ~ 'prev_period',
      startsWith(stat_name_1, '(9) Previous' ) ~ 'prev_period',
      startsWith(stat_name_1, '(10) Previous') ~ 'prev_period',
      startsWith(stat_name_2, '1958'         ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1957'         ) ~ 'cum_prev_year',

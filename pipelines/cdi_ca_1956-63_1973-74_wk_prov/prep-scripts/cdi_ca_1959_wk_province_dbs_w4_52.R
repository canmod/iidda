...
tidy_dataset = "cdi_ca_1959_wk_prov_dbs_w4_52"
...
behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 3, 32), between(col, 2, 50))
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_1, '(3) Report') ~ 'this_period',
      startsWith(stat_name_1, 'Previous'  ) ~ 'prev_period',
      startsWith(stat_name_2, '1959'      ) ~ 'cum_report_year',

...
tidy_dataset = "cdi_ca_1967_wk_prov_mcgill"
digitization = "cdi_ca_1967_wk_prov_mcgill"
metadata_path = "pipelines/cdi_ca_1964-67_wk_prov/tracking"
# ----------------------------------------
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1967'      ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1966'      ) ~ 'cum_prev_year',
    ...
    %>% mutate(icd_7_subclass = ifelse(disease_subclass == "Other  (", "036-039", icd_7_subclass)) # 2 numbers in brackets, first is not icd code
    %>% mutate(disease_subclass = ifelse(disease_subclass == "Other  (", "Other", disease_subclass)) # 2 numbers in brackets, first is not icd code
    # ------------------------------------------------------------------------
  )
}

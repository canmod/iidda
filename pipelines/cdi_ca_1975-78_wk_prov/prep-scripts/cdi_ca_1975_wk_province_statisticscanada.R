...
tidy_dataset = "cdi_ca_1975_wk_prov_statcan"
digitization = "cdi_ca_1975_wk_prov_statcan"
metadata_path = "pipelines/cdi_ca_1975-78_wk_prov/tracking"
# ----------------------------------------
...
behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 4, 29), between(col, 1, 41))
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    # icd codes
    %>% rename(icd_7 = icd)
    %>% mutate(icd_7_subclass = ifelse(is_subclass==FALSE, NA, icd_7))
    %>% mutate(icd_7 = ifelse(is_subclass, NA, icd_7))
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1975'      ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1974'      ) ~ 'cum_prev_year',
    ))

    %>% mutate(cases_statistic = paste0('cases_', cases_statistic))
    %>% relocate(cases, .after = last_col())
    %>% filter(!is.na(cases))
    %>% pivot_wider(id_cols = c(location, disease, disease_subclass, icd_7, icd_7_subclass),
...
get_periods = function(sheets, data, metadata) {
  (data
   ...
   %>% mutate(end_date = sub(
     ...
     "", end_date))
   %>% mutate(end_date = ifelse(sheet == "Dec20", "December 20, 1975", end_date))# Sheet "Dec20" accounts for a 2-week time period
   %>% mutate(end_date = mdy(trimws(end_date)))
   %>% mutate(start_date =
                end_date - days(freq_to_days(metadata$Source$frequency) - 1))
   %>% mutate(start_date =
                as.Date(ifelse(sheet == "Dec20", as.Date(start_date-days(7)), as.Date(start_date)))) # Sheet "Dec20" accounts for a 2-week time period

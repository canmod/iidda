...
tidy_dataset = "cdi_ca_1976_wk_prov_statcan"
digitization = "cdi_ca_1976_wk_prov_statcan"
...
behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 3, 28), between(col, 1, 41))
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      (startsWith(stat_name_1, 'Total'    ) & startsWith(stat_name_2, '1975')) ~ 'cum_report_year',
      (startsWith(stat_name_1, 'Total'    ) & startsWith(stat_name_2, '1976')) ~ 'cum_report_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1974')) ~ 'cum_prev_year',
      (is.na(stat_name_1) & startsWith(stat_name_2, '1975')) ~ 'cum_prev_year'
    ))

...
tidy_dataset = "cdi_ca_1978_wk_prov_statcan"
digitization = "cdi_ca_1978_wk_prov_statcan"
...
behead_sheet = function(focal_sheet, data) {
  (data
   %>% filter(sheet == focal_sheet, between(row, 3, 29), between(col, 1, 41))
...
clean_sheets = function(
  ...
  (beheaded_sheet
    ...
    %>% mutate(cases_statistic = case_when(
      ...
      startsWith(stat_name_2, '1978'      ) ~ 'cum_report_year',
      startsWith(stat_name_2, '1977'      ) ~ 'cum_prev_year',
    ))

...
tidy_dataset = "cdi_syphlatent_ca_1944-55_wk_prov"
digitization = "cdi_syphlatent_ca_1944-55_wk_prov"
...
missing_values = function(data){
  ...
  (filtered_data
   ...
   # timescales corresponding to the same period are empty
    ...
    # 1944: PEI, NS, NB, Ont, Sask, Alta, and BC are reporting weekly but not monthly until Dec 30
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' 
        & location %in% c('P.E.I', 'N.S.','N.B.', 'ONT.', 'SASK.', 'ALTA.', 'B.C.') 
        & cases_this_period %in% c('-', '—', '', 0)
    ...
    # 1944: Ontario not reporting weekly until Sept 2, when large case counts start appearing.

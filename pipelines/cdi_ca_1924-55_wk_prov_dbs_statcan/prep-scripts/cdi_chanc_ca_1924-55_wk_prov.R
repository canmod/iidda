...
tidy_dataset = "cdi_chanc_ca_1924-55_wk_prov"
digitization = "cdi_chanc_ca_1924-55_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  clean_weeks = (beheaded_sheet
                 ...
                 %>% unite(cases_this_period, c(numeric, character), sep='', remove = FALSE, na.rm=TRUE)
  ...
  rr = rbind(filter(clean_wkmt, week_ending != 'TOTAL'),
        case1,
        case2) %>% select(location, period_start_date, period_end_date, disease, cases_this_period)
  rr$collection_year = focal_sheet_info$year
  rr
}
...
missing_values = function(data){
  (data
   ...
   # Locations which have no data on any timescale for an entire year are removed
    %>% group_by(location, year = format(as.Date(period_end_date), "%Y"))
    %>% filter(!all(cases_this_period == ''))

    # If there are no reported cases and less than 25 dashes, remove that location and year from the dataset
    %>% mutate(dashes = sum(grepl('[-—]', cases_this_period) & time_scale == 'wk'))
    %>% mutate(casecount = sum(suppressWarnings(as.numeric(cases_this_period)), na.rm = TRUE))
    %>% filter(!(dashes <= 25 & casecount == 0))

    %>% ungroup()
    %>% select(-year, -dashes, -casecount)

    # Convert NAs in month totals to empty cells
    %>% mutate(cases_this_period = ifelse(is.na(cases_this_period), '', cases_this_period))

    # Get rid of subtotals in 1953 wk ending June 27 and 1954 wk ending June 26
    %>% filter(!(time_scale == 'mo' & as.Date(period_end_date, format="%B %d %Y") == as.Date("June 27 1953", format="%B %d %Y")))
    %>% filter(!(time_scale == 'mo' & as.Date(period_end_date, format="%B %d %Y") == as.Date("June 26 1954", format="%B %d %Y")))

    # Ontario and Canada reporting monthly but not weekly until June 28, 1924
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('ONT.', 'CANADA')  & as.Date(period_end_date, format="%B %d %Y") <= as.Date("June 28 1924", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Ontario and Canada reporting monthly but not weekly between Jan 1 and Jan 29, 1927
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('ONT.', 'CANADA')  & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("January 29 1927", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 1 1927", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # N.S., Sask, Can, Que reporting weekly but not monthly between Jan 1 and Dec 30 1944
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' & location %in% c('N.S.', 'SASK.', 'QUE.', 'CANADA')
        & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 30 1944", format="%B %d %Y")
        & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1944", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Before 1952, all other empty cells = 0. After 1952, '-' means 0, and empty cells are taken to be 'Not available'
    %>% mutate(cases_this_period = ifelse(
      as.Date(period_end_date, format="%B %d %Y") < as.Date("January 5 1952", format="%B %d %Y") & cases_this_period == "",
      0,
      ifelse(as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 5 1952", format="%B %d %Y") & cases_this_period == "",
             'Not available',
             cases_this_period)
    ))

    # Convert all dashes to 0

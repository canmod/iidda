...
tidy_dataset = "cdi_dysbac_ca_1924-55_wk_prov"
digitization = "cdi_dysbac_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Alberta starts reporting in 1936
   %>% mutate(
   ...
   # Manitoba starts reporting in 1942
   %>% mutate(
     cases_this_period = ifelse(
       location == 'MAN.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1942", format="%B %d %Y")
       & cases_this_period == '',
       'Not available',
       ifelse(
   ...
   # NB starts reporting in 1955
   %>% mutate(
     cases_this_period = ifelse(
       location == 'N.B.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1955", format="%B %d %Y")
       & cases_this_period == '',
       'Not available',
       ifelse(
   ...
   # Newfoundland starts reporting in 1951
   %>% mutate(
     cases_this_period = ifelse(
       location == 'NFLD.'
   ...
   # Ontario starts reporting in 1931
   %>% mutate(
     cases_this_period = ifelse(
       location == 'ONT.'
   ...
   # Quebec reporting in 1934, 1939 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # not available before 1934
       (location == 'QUE.'
        & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1934", format="%B %d %Y")
        & cases_this_period == '')
       # not available from 1935-38
       | (location == 'QUE.'
   ...
   # Saskatchewan reporting in 1924, 1926, 1928, 1931 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # 1924: empty = 0
       (location == 'SASK.'
       ...
       | (location == 'SASK.'
          ...
          & cases_this_period == '')
       # 1931 onwards: empty = 0
       | (location == 'SASK.'
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1931", format="%B %d %Y")
          & cases_this_period == '')
       , 0
       # Saskatchewan is otherwise not available
       , ifelse(
   ...
   %>% mutate(cases_this_period = ifelse(is.na(cases_this_period), '', cases_this_period))
    ...
    # B.C. and Canada reporting monthly but not weekly between June 7 1930 and Dec 26 1931
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('B.C.', 'CANADA') & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 26 1931", format="%B %d %Y")
        & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("June 7 1930", format="%B %d %Y") & cases_this_period == ""

...
tidy_dataset = "cdi_tbunspec_ca_1933-55_wk_prov"
digitization = "cdi_tbunspec_ca_1933-55_wk_prov"
...
behead_sheet = function(focal_sheet, data) {
  (data
   ...
   %>% mutate(fixed_statname = ifelse(focal_sheet == 1953 & location == 'CANADA',
                                      ifelse(is.na(location), 'wk', 'mo'),
                                             ifelse(is.na(location), 'mo', 'wk')))
...
missing_values = function(data){
  (data
   ...
   # Quebec weekly reports not available before may 1939
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk'
       & cases_this_period %in% c('-', '—', '',0)
       & location == 'QUE.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("May 6 1939", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("Jan 7 1939", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))

   # Quebec monthly reports not available in 1939 before April 1
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & cases_this_period %in% c('-', '—', '',0)
       & location == 'QUE.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("April 1 1939", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("Jan 7 1939", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))

   # PEI weekly not available in 1948
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk'
       & cases_this_period %in% c('-', '—', '',0)
   ...
   # Ontario monthly reports not available in 1952
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & cases_this_period %in% c('-', '—', '',0)
   ...
   # Data is not available between January 7 - December 30 1939 in canada
   %>% mutate(
     cases_this_period = ifelse(
       location == 'CANADA'
   ...
   |> mutate(
      cases_this_period = ifelse(
          location == "QUE."
          & collection_year %in% c("1939")
          & as.Date(period_end_date) < as.Date("1939-05-01"),
          "Not available",
          cases_this_period
      )
   )
   ...
   |> mutate(
      cases_this_period = ifelse(
          location == "P.E.I."
          & collection_year %in% c("1943")
          & as.Date(period_end_date) > as.Date("1943-10-01"),
          "Not available",
          cases_this_period
      )
   )
   ...
   |> mutate(
      cases_this_period = ifelse(
          location %in% c("P.E.I.", "P.E.I")
          & collection_year %in% c("1944", "1945", "1947")
          & time_scale != "yr",
          "Not available",
          cases_this_period
      )
   ...
   |> mutate(
      cases_this_period = ifelse(
          location == "ONT."
          & collection_year %in% c("1943", "1944")
          & time_scale != "yr",
          "Not available",
          cases_this_period
      )
   ...
   |> mutate(
      cases_this_period = ifelse(
          location == "P.E.I"
          & collection_year == "1943"
          & as.Date(period_end_date) > as.Date("1943-09-30"),
          "Not available",
          cases_this_period
      )
   )

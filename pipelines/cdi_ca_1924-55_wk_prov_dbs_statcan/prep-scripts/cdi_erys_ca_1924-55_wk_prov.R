...
tidy_dataset = "cdi_erys_ca_1924-55_wk_prov"
digitization = "cdi_erys_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   %>% mutate(cases_this_period = ifelse(is.na(cases_this_period), '', cases_this_period))
    ...
    # B.C. and Canada reporting monthly but not weekly between June 7 1930 and Dec 31, 1932
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('MAN.', 'QUE.', 'B.C.', 'CANADA') & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y")
        & cases_this_period == "",
        'Not available',
        cases_this_period
      ))
    
    # Before 1948, all other empty cells = 0. After 1948, '-' means 0, and empty cells are taken to be 'Not available'
    %>% mutate(cases_this_period = ifelse(
      as.Date(period_end_date, format="%B %d %Y") < as.Date("January 3 1948", format="%B %d %Y") & cases_this_period == "", 
      0, 
      ifelse(as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 3 1948", format="%B %d %Y") & cases_this_period == "", 
             'Not available', 
             cases_this_period)
    ))

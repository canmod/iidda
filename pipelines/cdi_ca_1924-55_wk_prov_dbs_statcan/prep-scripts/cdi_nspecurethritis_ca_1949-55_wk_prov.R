...
tidy_dataset = "cdi_nspecurethritis_ca_1949-55_wk_prov"
digitization = "cdi_nspecurethritis_ca_1949-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Before 1951, all other empty cells = 0. After 1951, '-' means 0, and empty cells are taken to be 'Not available'
   %>% mutate(cases_this_period = ifelse(
     as.Date(period_end_date, format="%B %d %Y") < as.Date("January 6 1951", format="%B %d %Y") & cases_this_period == "", 
     0, 
     ifelse(as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1951", format="%B %d %Y") & cases_this_period == "", 
            'Not available', 
            cases_this_period)
   ))

...
tidy_dataset = "cdi_dysunspec_ca_1927-55_wk_prov"
digitization = "cdi_dysunspec_ca_1927-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   %>% mutate(cases_this_period = ifelse(is.na(cases_this_period), '', cases_this_period))
    ...
    # Convert '-' to 0, empty to unavailable
    %>% mutate(cases_this_period = ifelse(cases_this_period %in% c("-", '—'), 0, cases_this_period))
   
    %>% mutate(cases_this_period = ifelse(cases_this_period == '', 'Not available', cases_this_period))
  )
}

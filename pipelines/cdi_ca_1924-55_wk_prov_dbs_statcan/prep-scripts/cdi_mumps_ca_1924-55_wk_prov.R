...
tidy_dataset = "cdi_mumps_ca_1924-55_wk_prov"
digitization = "cdi_mumps_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Ontario and Alberta are reporting monthly but not weekly until June 28 , 1924
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('ONT.', 'ALTA.')
    ...
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('B.C.', 'QUE.', 'MAN.')
        & cases_this_period %in% c('', 0)
    ...
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('P.E.I.', 'P.E.I')
    ...
    # in 1939, Quebec has no data until April 29 where it starts reporting large numbers
    # Convert weekly and monthly totals in 1934 until April 29 for Quebec to not available
    %>% mutate(
      cases_this_period = ifelse(
        time_scale %in% c('wk', 'mo') & location == 'QUE.'
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 7 1939", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("April 29 1939", format="%B %d %Y")
        & cases_this_period %in% c('', 0),
        'Not available',
        cases_this_period
      ))

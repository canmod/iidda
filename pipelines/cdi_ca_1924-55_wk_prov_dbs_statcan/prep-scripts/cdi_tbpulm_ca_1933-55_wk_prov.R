...
tidy_dataset = "cdi_tbpulm_ca_1933-55_wk_prov"
digitization = "cdi_tbpulm_ca_1933-55_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  ...
  case2 =  (clean_wkmt
            ...
            %>% filter(none_less_than_max(cases_this_period))
...
missing_values = function(data){
  (data
   ...
   # timescales corresponding to the same period are empty
    ...
    # PEI reporting monthly but not reporting weekly in 1946
    %>% mutate(
      cases_this_period = ifelse(
        location %in% c('P.E.I', 'P.E.I.')
        & time_scale == 'wk'
    ...
    # PEI reporting monthly but not reporting weekly from 1949-51
    %>% mutate(
      cases_this_period = ifelse(
        location %in% c('P.E.I', 'P.E.I.')
        & time_scale == 'wk'
   ...
   # Data is not available between May 13 - December 30 1939
   %>% mutate(
     cases_this_period = ifelse(
       as.Date(period_end_date, format="%B %d %Y") >= as.Date("May 13 1939", format="%B %d %Y")

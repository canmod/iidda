...
tidy_dataset = "cdi_dysam_ca_1927-55_wk_prov"
digitization = "cdi_dysam_ca_1927-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Manitoba starts reporting in 1933
   %>% mutate(
     cases_this_period = ifelse(
       location == 'MAN.'
   ...
   # Newfoundland starts reporting in 1953
   %>% mutate(
     cases_this_period = ifelse(
       location == 'NFLD.'
   ...
   %>% mutate(
     cases_this_period = ifelse(
       ...
       'Not available',
       ifelse(
         location == 'P.E.I' & cases_this_period == '' 
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1952", format="%B %d %Y") ,
         '0',
         cases_this_period)))
   
   # Quebec is reporting in 1934, 1939-42, 1948-53
   %>% mutate(
     cases_this_period = ifelse(
       # 1934: empty = 0 
       (location == 'QUE.' 
       ...
       | (location == 'QUE.'
          ...
          & cases_this_period == '')
       , 0
       # Quebec is otherwise not available
       , ifelse(
         location == 'QUE.' & cases_this_period == '',
         'Not available',
         cases_this_period)))
   
   # Saskatchewan is reporting in 1927, 1931 onward
   %>% mutate(
     cases_this_period = ifelse(
       # not available before 1927
       (location == 'SASK.' 
        & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1927", format="%B %d %Y")
        & cases_this_period == '')
       # not available from 1928-30
       | (location == 'SASK.'
          & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1931", format="%B %d %Y")
   ...
   %>% mutate(cases_this_period = ifelse(is.na(cases_this_period), '', cases_this_period))
    ...
    # B.C. and Canada reporting monthly but not weekly until Dec 27, 1930
    %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' 
       & location %in% c('B.C.', 'CANADA') & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 27 1930", format="%B %d %Y")

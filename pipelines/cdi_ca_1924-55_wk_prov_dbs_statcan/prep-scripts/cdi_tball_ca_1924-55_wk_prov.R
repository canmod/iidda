...
# This file will have a different period matching file than other files since
# four weekly periods start in 1951, whereas four weekly periods usually start
# in 1952 for other files.

# ----------------------------------------
# Information for Locating Metadata
tidy_dataset = "cdi_tball_ca_1924-55_wk_prov"
digitization = "cdi_tball_ca_1924-55_wk_prov"
...
# For the years 1924-50, monthly totals are calculated over uneven months. This switches from 1951-55 to being calculated over 4 weekly periods.
...
one_four_weekly_period = local({
  ...
  startdate1951_53 = ymd(19510106) - 6 + days(28 * 0:39)
  startdate54_55 = ymd(19540131) + days(28*0:24)
  period_start_date = as.character(append(startdate1951_53, startdate54_55))

  enddate1951_53 = head(startdate1951_53, -1) + days(27)
  enddate1951_53 = append(enddate1951_53, ymd(19540130))
  enddate54_55 = startdate54_55 + days(27)
  period_end_date = as.character(append(enddate1951_53, enddate54_55))
...
clean_sheet = function(beheaded_sheet, focal_sheet_info){
  ...
  # week_ending_day_of_week = (clean_weeks
  #   |> filter(!iidda::is_empty(week_ending_date))  ## maybe filter out non-date patterns instead of empties?
  #   |> group_by(month)
  #   |> summarise(
  #     wday = lubridate::wday(max(week_ending_date)))
  #   |> ungroup()
  #   |> distinct(wday)
  #   |> pull(wday)
  #   |> as.integer()
  # )
  # stopifnot(identical(week_ending_day_of_week), 7L)
  # clean_months = (clean_weeks
  #   |> filter(stat_name == "mo", !iidda::is_empty(cases_this_period))
  #   |> mutate(month_ending_date = rollforward_to_same_weekday(week_ending_date))
  #   |> mutate(month_starting_date = rollbackward_to_same_weekday(week_ending_date) - days(6L))
  #   |> group_by(location, month_starting_date, month_ending_date)
  #   |> summarise(cases_this_period = cases_this_period)
  #   |> ungroup()
  # )

  # Month start dates for years less than 1951 are calculated differently than 1951-55.
  if (as.numeric(focal_sheet_info$year) < 1951) {
    clean_wkmt =
      (clean_weeks
       %>% group_by(month)
...
missing_values = function(data){
  (data
   ...
   # Newfoundland starts reporting in July, 1949
   %>% mutate(
   ...
   # PEI starts reporting in 1929
   %>% mutate(
     cases_this_period = ifelse(
       location %in% c('P.E.I', 'P.E.I.')
       & cases_this_period == ''
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1929", format="%B %d %Y") ,
       'Not available',
       ifelse(
         location %in% c('P.E.I', 'P.E.I.') & cases_this_period == ''
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1929", format="%B %d %Y") ,
         '0',
         cases_this_period)))
   ...
   # timescales corresponding to the same period are empty
    ...
    # Ontario and Alberta are reporting monthly but not weekly until June 28, 1924
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk'
        & location %in% c('ONT.', 'ALTA.')
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("June 28 1924", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # B.C., Manitoba, and Quebec are reporting monthly but not weekly until Dec 31, 1932
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & location %in% c('B.C.', 'QUE.', 'MAN.')
        & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    #  PEI is reporting monthly but not weekly in 1932
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & location %in% c('P.E.I.', 'P.E.I')
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 2 1932", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Canada reporting yearly but not reporting weekly or monthly for 1933
    %>% mutate(
      cases_this_period = ifelse(
        location == 'CANADA'
        & cases_this_period %in% c('-', '—', '',0)
        & time_scale %in% c('wk', 'mo')
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 7 1933", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 30 1933", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Canada reporting yearly but not reporting weekly or monthly for 1935-36
    %>% mutate(
      cases_this_period = ifelse(
        location == 'CANADA'
        & cases_this_period %in% c('-', '—', '',0)
        & time_scale %in% c('wk', 'mo')
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 5 1935", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 26 1936", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Canada and Alberta reporting yearly but not reporting weekly or monthly for 1938
    %>% mutate(
      cases_this_period = ifelse(
        location %in% c('CANADA', 'ALTA.')
        & time_scale %in% c('wk', 'mo')
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 1 1938", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1938", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Canada reporting monthly but not weekly for 1939
    %>% mutate(
      cases_this_period = ifelse(
        location == 'CANADA'
        & time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 7 1939", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 30 1939", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Alberta reporting yearly but not reporting weekly or monthly for 1939
    %>% mutate(
      cases_this_period = ifelse(
        location == 'ALTA.'
        & time_scale %in% c('wk', 'mo')
    ...
    # Alberta reporting monthly but not reporting weekly for 1940-41
    %>% mutate(
      cases_this_period = ifelse(
        location == 'ALTA.'
        & time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1940", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 27 1941", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # BC reporting monthly but not reporting weekly for 1940-42
    %>% mutate(
      cases_this_period = ifelse(
        location == 'B.C.'
        & cases_this_period %in% c('-', '—', '',0)
        & time_scale == 'wk'
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1940", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("February 28 1942", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # Canada reporting monthly but not reporting weekly for 1940-42
    %>% mutate(
      cases_this_period = ifelse(
        location == 'CANADA'
        & time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1940", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("June 27 1942", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # in 1943, PEI goes from reporting many cases to having empty cells, since we are unable to
    # decipher whether empty here = 0 or empty = not available, remove these weeks
    %>% filter(!((location == 'P.E.I')
                 & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 25 1943", format="%B %d %Y")
                 & as.Date(period_end_date, format="%B %d %Y") >= as.Date("September 25 1943", format="%B %d %Y")
    ))

    # PEI is reporting yearly but not weekly or monthly for 1944-45
    %>% mutate(
      cases_this_period = ifelse(
        location == 'P.E.I'
        & time_scale %in% c('wk', 'mo')
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 1 1944", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 29 1945", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # PEI is reporting monthly but not weekly in 1946
    %>% mutate(
      cases_this_period = ifelse(
        location == 'P.E.I'
        & time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 5 1946", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 28 1946", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # PEI is reporting yearly but not weekly or monthly for 1947
    %>% mutate(
      cases_this_period = ifelse(
        location == 'P.E.I'
        & time_scale %in% c('wk', 'mo')
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 4 1947", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 27 1947", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # PEI is reporting monthly but not weekly for 1948
    %>% mutate(
      cases_this_period = ifelse(
        location == 'P.E.I'
        & time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 3 1948", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 25 1948", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

    # PEI is reporting monthly but not reporting weekly for 1949-51
    %>% mutate(
      cases_this_period = ifelse(
        location == 'P.E.I'
        & time_scale == 'wk'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 1 1949", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 29 1951", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))
   ...
   |> mutate(
      cases_this_period = ifelse(
          location == "B.C."
          & time_scale == "wk"
          & collection_year == "1939"
          & as.Date(period_end_date) > as.Date("1939-04-29"),
          "Not available",
          cases_this_period
      )
   )

...
tidy_dataset = "cdi_wc_ca_1924-55_wk_prov"
digitization = "cdi_wc_ca_1924-55_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  clean_weeks = (beheaded_sheet
                 ...
                 %>% mutate(cases_this_period = gsub("\\(1\\)", "", cases_this_period))
...
combine_sheets=function(cleaned_sheets){
  ...
  monthly=(combined
           %>% filter(period_end_date - period_start_date > 7 & period_end_date - period_start_date < 50)
...
missing_values = function(data){
  (data
   ...
   %>% mutate(
      cases_this_period = ifelse(
          # this total includes a little bit (16) included after the
          # last full week, so removing this bit from the total.
          time_scale == "yr" & location == "N.S." & collection_year == "1935"
        , as.character(410L - 16L), cases_this_period
      )
   )

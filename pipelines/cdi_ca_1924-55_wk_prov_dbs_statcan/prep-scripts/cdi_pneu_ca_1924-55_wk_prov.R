...
tidy_dataset = "cdi_pneu_ca_1924-55_wk_prov"
digitization = "cdi_pneu_ca_1924-55_wk_prov"
...
missing_values = function(data){
  ...
  (filtered_data
    # Get rid of BC mt/wk totals from Jan 5 1929 - Oct 26 1929
    %>% filter(!(location == 'B.C.'
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 5 1929", format="%B %d %Y") &
         as.Date(period_end_date, format="%B %d %Y") <= as.Date("October 26 1929", format="%B %d %Y")))
    
    # B.C. is reporting monthly but not weekly from Nov 2 1929 - Dec 31 1932 
    %>% mutate(
    ...
    # In 1932 , PEI is reporting monthly but not weekly from Jan 2 - Dec 31
    %>% mutate(
      cases_this_period = ifelse(

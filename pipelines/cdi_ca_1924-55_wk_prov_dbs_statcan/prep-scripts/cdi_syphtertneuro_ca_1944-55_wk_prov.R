...
tidy_dataset = "cdi_syphtertneuro_ca_1944-55_wk_prov"
digitization = "cdi_syphtertneuro_ca_1944-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # timescales corresponding to the same period are empty
    ...
    # 1944: PEI, NB, Ont, Sask, Alta, BC are reporting weekly but not monthly until Dec 30
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' 
        & location %in% c('P.E.I', 'P.E.I.','N.B.', 'ONT.', 'SASK.', 'ALTA.', 'B.C.') 
        & cases_this_period %in% c('-', '—', '',0)

...
tidy_dataset = "cdi_rub_ca_1924-55_wk_prov"
digitization = "cdi_rub_ca_1924-55_wk_prov"
...
missing_values = function(data){
  ...
  # Manitoba starts reporting in 1931
  %>% mutate(
  ...
  # NB reporting in 1932, 1942, 1944, 1950, 1952 onwards
  %>% mutate(
    cases_this_period = ifelse(
      # 1932: empty = 0
      (location == 'N.B.'
  ...
  %>% mutate(
    cases_this_period = ifelse(
      ...
      (location %in% c('P.E.I.', 'P.E.I')
       ...
       & cases_this_period == '')
      , 0
      # PEI is otherwise not available
      , ifelse(
        location %in% c('P.E.I.', 'P.E.I') & cases_this_period == '',
        'Not available',
        cases_this_period)))

  # Quebec starts reporting in 1928
  %>% mutate(
    cases_this_period = ifelse(
      location == 'QUE.'

...
tidy_dataset = "cdi_yf_ca_1945-1955_wk_prov"
digitization = "cdi_yf_ca_1945-1955_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  ...
  if(as.numeric(focal_sheet_info$year) < 1952){
    (clean_no_mt
     ...
     %>% ungroup()
     %>% mutate(period_end_date=ifelse(stat_name=='mt', month_end_date, period_end_date))
     ...
     %>% select(location, period_start_date, period_end_date, disease,cases_this_period)
    )

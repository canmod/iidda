...
tidy_dataset = "cdi_malar_ca_1929-55_wk_prov"
digitization = "cdi_malar_ca_1929-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Manitoba reporting 1945-47, 1949 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # 1945-47: empty = 0
       (location == 'MAN.' 
   ...
   # NB reporting 1949, 1953 onwards
   %>% mutate(
     cases_this_period = ifelse(
   ...
   # Newfoundland starts reporting in 1952
   %>% mutate(
     cases_this_period = ifelse(
       location == 'NFLD.'
   ...
   # Ontario reporting in 1929, 1931 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # not available before 1929
       (location == 'ONT.' 
        & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1929", format="%B %d %Y")
        & cases_this_period == '')
   ...
   # PEI reporting in 1929, 1935-48, 1952 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # 1929: empty = 0 
   ...
   # Quebec starts reporting in 1949
   %>% mutate(
     cases_this_period = ifelse(
       location == 'QUE.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1949", format="%B %d %Y") 
   ...
   # Saskatchewan starts reporting in 1931
   %>% mutate(
     cases_this_period = ifelse(
       location == 'SASK.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1931", format="%B %d %Y") 
   ...
   # BC and Canada reporting monthly but not weekly until December 26 1931
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' & location %in% c('B.C.', 'CANADA') & cases_this_period %in% c('', 0)
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 26 1931", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))

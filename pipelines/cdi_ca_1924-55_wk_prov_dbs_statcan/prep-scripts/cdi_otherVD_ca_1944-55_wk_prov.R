...
# **NOTE: this is a script for only the first two sheets. The last sheet is
# separated by age/sex and will need to be run through a different script.
# cleaned_sheets gives warnings for period_start_date and month_start_date.
# This may be because some cases have numbers in brackets
# relating to a comment before the case count.

# ----------------------------------------
# Information for Locating Metadata
tidy_dataset = "cdi_otherVD_ca_1944-55_wk_prov"
digitization = "cdi_otherVD_ca_1944-55_wk_prov"
...
get_sheet_names =function(data){
  ...
  c("1944", "1945")  ## hack given note at the top of this file
}
...
# For the years 1924-52, monthly totals are calculated over uneven months. This switches from 1953-55 to being calculated over 4 weekly periods.
...
one_four_weekly_period = local({
  ...
  startdate1953 = ymd(19530103) - 6 + days(28 * 0:13)
  startdate54_55 = ymd(19540131) + days(28*0:24)
  period_start_date = as.character(append(startdate1953, startdate54_55))

  enddate1953 = head(startdate1953, -1) + days(27)
  enddate1953 = append(enddate1953, ymd(19540130))
  enddate54_55 = startdate54_55 + days(27)
  period_end_date = as.character(append(enddate1953, enddate54_55))
...
combine_sheets=function(cleaned_sheets){
  ...
  check = select(monthly, location, period_start_date, disease)
...
missing_values = function(data){
  (data
   ...
   # Monthly reports for 1944 are not available
   %>% mutate(

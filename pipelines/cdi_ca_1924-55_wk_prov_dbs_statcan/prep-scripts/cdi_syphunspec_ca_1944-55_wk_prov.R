...
tidy_dataset = "cdi_syphunspec_ca_1944-55_wk_prov"
digitization = "cdi_syphunspec_ca_1944-55_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  clean_weeks = (beheaded_sheet
                 ...
                 # Filtering out subtotal in month column for week ending July 2, 1955
                 %>% filter(!(stat_name == 'mo' & week_ending_date == '1955-07-02'))
  )
...
missing_values = function(data){
  (data
   ...
   # timescales corresponding to the same period are empty
    ...
    # 1944: Canada and Quebec are reporting monthly but not weekly until Sept 30
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' 
        & location == 'CANADA' 
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("September 30 1944", format="%B %d %Y"),
        'Not available',
        cases_this_period
      )) 
    
    # 1944: PEI, NS, NB, Ont, Man, Sask, Alta, and BC are reporting weekly but not monthly until Dec 30
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' 
        & cases_this_period %in% c('-', '—', '',0)
        & location %in% c('P.E.I','N.B.','N.S.', 'ONT.', 'MAN.','SASK.', 'ALTA.', 'B.C.') 
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 30 1944", format="%B %d %Y"),
        'Not available',
        cases_this_period
      )) 
    
    # 1944: Ontario not reporting weekly until Sept 30, when large case counts start appearing.
    # Can't tell whether empty = 0 or empty = not available, so removing these weekly and monthly counts from the data set.
    %>% filter(!(location %in% c('ONT.', 'MAN.') & 
                   as.Date(period_end_date, format="%B %d %Y") <= as.Date("August 26 1944", format="%B %d %Y")
    ))
    
    # In 1949, there are weekly cases but no monthly cases in all locations --> make monthly = not available
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' 
        & cases_this_period %in% c('-', '—', '',0)

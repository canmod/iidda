...
tidy_dataset = "cdi_poliounspec_ca_1949-55_wk_prov"
digitization = "cdi_poliounspec_ca_1949-55_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  ...
  (rbind(filter(clean_wkmt, week_ending != 'TOTAL'),
        case1,
        case2)
  %>% select(location, period_start_date, period_end_date, disease, cases_this_period, collection_year)
  %>% mutate(cases_this_period = if_else(grepl("\\*", cases_this_period),
                                       gsub("\\*", "", cases_this_period),
                                       cases_this_period))
  )

...
# (week_ending in 1942 is Aug 15, usually is Aug 14)

# ----------------------------------------
# Information for Locating Metadata
tidy_dataset = "cdi_typh_ca_1924-52_wk_prov"
digitization = "cdi_typh_ca_1924-52_wk_prov"
...
missing_values = function(data){
  (data
   ...
   %>% mutate(
    ...
    # In 1952, all locations are reporting weekly but are not reporting monthly
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo'
        & cases_this_period %in% c('-', '—', '',0)

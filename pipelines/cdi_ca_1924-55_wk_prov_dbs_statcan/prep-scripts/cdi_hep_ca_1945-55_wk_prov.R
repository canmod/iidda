...
tidy_dataset = "cdi_hep_ca_1945-55_wk_prov"
digitization = "cdi_hep_ca_1945-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Alberta reporting in 1947, 1949 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # not available before 1947
       (location == 'ALTA.' 
   ...
   # Manitoba reporting from 1952 onwards
   %>% mutate(
     cases_this_period = ifelse(
       location == 'MAN.'
   ...
   # NB reporting from 1953 onwards
   %>% mutate(
     cases_this_period = ifelse(
       location == 'N.B.'
   ...
   # Newfoundland reporting from july 1949 onwards
   %>% mutate(
     cases_this_period = ifelse(
       location == 'NFLD.'
   ...
   # NS reporting until 1946, 1948-55
   %>% mutate(
     cases_this_period = ifelse(
       # up until 1946, empty = 0 
       (location == 'N.S.' 
        & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1947", format="%B %d %Y")
        & cases_this_period == '')
       # 1948-55: empty = 0
   ...
   # Saskatchewan reporting 1931 onwards
   %>% mutate(
     cases_this_period = ifelse(
       location == 'SASK.'
   ...
   # BC and Canada are reporting weekly but not monthly from jan 6 1945 until dec 29 1945
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo' 
       & location %in% c('B.C.', 'CANADA') 
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 29 1945", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1945", format="%B %d %Y")

...
# Scans are missing the week ending Jan 2, 1954.
# This causes the time scale matching file for this disease to differ from the others.

# ----------------------------------------
# Information for Locating Metadata
tidy_dataset = "cdi_polio_ca_1924-55_wk_prov"
digitization = "cdi_polio_ca_1924-55_wk_prov"
metadata_path = "pipelines/cdi_ca_1924-55_wk_prov_dbs_statcan/tracking"
# ----------------------------------------

options(conflicts.policy = list(warn.conflicts = FALSE))
# remotes::install_github('canmtd-eidm/iidda-tools', subdir = 'R/iidda')
library(plyr)
library(dplyr)
library(iidda)
library(tidyxl)
...
# For the years 1924-51, mtnthly totals are calculated over uneven mtnths. This switches from 1952-55 to being calculated over 4 weekly periods.
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  clean_weeks = (beheaded_sheet
                 ...
                 %>% mutate(mtnth=str_extract(week_ending, "[A-z]+"))
                 %>% mutate(day=str_extract(week_ending, "[1-3]?[0-9]{1}"))
                 %>% mutate(mtnth=na.locf(mtnth))
                 %>% unite(cases_this_period, c(numeric, character), sep='', na.rm=TRUE)
                 %>% mutate(location=na.locf(location))

                 %>% filter(!is.na(week_ending))

                 # Fixing case where mtnths are abbreviated to 1 letter
                 %>% mutate(mtnth=case_when((mtnth == "J" & row < 10) ~ 'Jan',
                                            (mtnth == "F") ~ 'Feb',
                                            (mtnth == "M" & row > 10 & row < 20) ~ 'Mar',
                                            (mtnth =="A" & row > 15 & row < 25) ~ 'Apr',
                                            (mtnth =="M" & row > 20 & row < 30) ~ 'May',
                                            (mtnth =="J" & row > 25 & row <= 30) ~ 'Jun',
                                            (mtnth =="J" & row > 30 & row < 40) ~ 'Jul',
                                            (mtnth =="A" & row > 30 & row < 40) ~ 'Aug',
                                            (mtnth =="S") ~ 'Sep',
                                            (mtnth =="O") ~ 'Oct',
                                            (mtnth =="N") ~ 'Nov',
                                            (mtnth =="D") ~ 'Dec',
                 ...
                 %>% mutate(mtnth=strtrim(.$mtnth, 3))
                 %>% mutate(unite(.,'week_ending_date', c(day, mtnth, year), sep=''))

                 # %>% unite('week_ending_date', c(day, mtnth, year), sep ='', remtve=FALSE)
                 ...
                 # Year end date
                 %>% mutate(period_end_date = ifelse(mtnth == 'TOT', as.character(max(week_ending_date, na.rm=TRUE)), period_end_date))
                 ...
                 # Year start date
                 %>% mutate(period_start_date=ifelse(mtnth=='TOT', as.character(min(as.Date(period_start_date), na.rm=TRUE)), period_start_date))

                 # Manually fixing year start date for 1955, since Jan 1 1955 is included in 1954 sheet
                 %>% mutate(period_start_date = ifelse(period_start_date == '1954-12-26' & mtnth == 'TOT', '1955-01-02', period_start_date))
  )

  # mtnth start dates for years less than 1952 are calculated differently than 1952-55.
  if(as.numeric(focal_sheet_info$year) < 1952){
    clean_wkmt =
      (clean_weeks
       %>% group_by(mtnth)
       %>% mutate(mtnth_end_date=ifelse(mtnth!='TOT', as.character(max(week_ending_date, na.rm=TRUE)), ''))
       %>% ungroup()
       %>% mutate(period_end_date=ifelse(mtnth!='TOT' & stat_name=='mo', mtnth_end_date, period_end_date))# important!

       %>% group_by(mtnth)
       %>% mutate(mtnth_start_date=as.character(min(as.Date(period_start_date), na.rm=TRUE)))
       %>% ungroup()
       %>% mutate(period_start_date=ifelse(stat_name=='mo', mtnth_start_date, period_start_date))

       %>% select(-mtnth_start_date, -mtnth_end_date)
      )
    ...
    clean_wkmt =
      (clean_weeks
       ...
       %>% mutate(period_start_date = ifelse(stat_name == 'mo' & mtnth!= "TOT", as.character(four_weekly_period$start_date(index)), period_start_date))
       %>% mutate(period_end_date = ifelse(stat_name == 'mo'& mtnth!= "TOT", as.character(four_weekly_period$end_date(index)), period_end_date))
       %>% select(-index)
      )
  }
  ...
  case2 =  (clean_wkmt
            ...
            # if yearly case in wk/mt are different and one is empty, remtve the empty one
            %>% filter(!all(cases_this_period == first(cases_this_period)))
...
combine_sheets=function(cleaned_sheets){
  ...
  # Filtering out duplicated mtnth values.
  mtnthly=(combined
           ...
           # Sometimes mtnthly total is last value for the period, sometimes it is the first value; so need to na.nocb and na.locf.
  ...
  # Checking if there are multiple case totals for the same mtnth
  check = select(mtnthly, location, period_start_date, disease, collection_year)
  duplicatemtnths = filter(mtnthly, duplicated(check)==TRUE)
  if(empty(duplicatemtnths) == FALSE){ message("Warning: the following multiple case totals have been included for the same mtnth")
    print(duplicatemtnths)}

  # Filtering for only weekly and yearly (and quarterly) data; to be joined back with cleaned mtnthly data.
  data_notmtnthly = (combined
                     %>% filter(!period_end_date - period_start_date > 7 | !period_end_date - period_start_date < 35)
  )

  rbind(data_notmtnthly, mtnthly) %>% rename(historical_disease = disease)
}
...
missing_values = function(data){
  (data
   ...
   # Alberta not reporting in 1925
   %>% mutate(
     cases_this_period = ifelse(
       location == 'ALTA.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1926", format="%B %d %Y")
   ...
   # Ontario starts reporting in 1925
   %>% mutate(
     cases_this_period = ifelse(
       location == 'ONT.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1925", format="%B %d %Y")
       & cases_this_period == '',
       'Not available',
       ifelse(
   ...
   # N.S, N.B., Sask., and Alta. all have weekly data but no mtnthly data for wk ending <= Dec 31 1932
   # Convert empty mtnth cells to not available for these groups.
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & location %in% c('N.S.', 'N.B.', 'SASK.', 'ALTA.')
   ...
   # Que., Man., and Can. all have mtnthly data but no weekly data for wk ending <= Dec 31 1932
   # Convert empty week cells to 'not available' for these groups.
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk'
       & location %in% c('QUE.', 'CANADA', 'MAN.')
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '',0),
       'Not available',
       cases_this_period
   ...
   # B.C. has monthly data but no weekly data for wk ending <= Dec 26 1925
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk'
       & location == 'B.C.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 26 1925", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '',0),
       'Not available',
       cases_this_period
     ))

   # B.C. has weekly data but no monthly data between April 24 1926 and May 29 1926
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo' & location == 'B.C.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("May 29 1926", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("April 24 1926", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '',0),
       'Not available',
       cases_this_period
     ))

   # B.C. has monthly data but no weekly data between Jan 1 1927 and Dec 31 1932
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' & location == 'B.C.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y")
   ...
   # In 1932, between July 2 and Oct 29 , P.E.I. has monthly data but no weekly data
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' & location == 'P.E.I'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("July 2 1932", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("October 29 1932", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '',0),
       'Not available',
       cases_this_period
     ))

   # From Jan 7 1933 until Dec 26 1936, all provinces have weekly data but no monthly data
   # (except Manitoba, which has weekly data and no monthly data until Dec 28 1935)
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo' & location != 'MAN.'
   ...
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo' & location == 'MAN.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 28 1935", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 7 1933", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '',0),
       'Not available',
       cases_this_period
   ...
   # Convert all dashes and empty cells to 0

...
tidy_dataset = "cdi_bot_ca_1933-55_wk_prov"
digitization = "cdi_bot_ca_1933-55_wk_prov"
...
get_sheet_names = function(data){
  grep("^19[3-5]+[0-9]+", unique(data$sheet), value = TRUE)
}

# Extract year and disease info from each sheet (in row 2) using regular expressions. Format changes once Newfoundland is added (1950)
get_sheet_info = function(focal_sheet, data){
  year = (data
   %>% filter(row == 2, sheet == focal_sheet)
   %>% .$numeric
   %>% grep('19[3-5]+[0-9]+', . , value = TRUE))
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  ...
  # (clean_weeks
  #   |> filter(grepl("[0-9]{4}-[0-9]{2}-[0-9]{2}", week_ending_date))
  #   |> group_by(month)
  #   |> summarise(
  #       period_start_date = min(period_end_date)
  ...
  intersect(filter(clean_wkmt, week_ending != 'TOTAL'), case2)
...
combine_sheets=function(cleaned_sheets){
  ...
  duplicate_maxvals = (duplicatemonths
  ...
  monthly = anti_join(monthly, duplicate_maxvals)
...
missing_values = function(data){
  (data
   ...
   # NB starts reporting in 1952, before which empty = Not available
   %>% mutate(
   ...
   # NL, Sask start reporting in 1953
   %>% mutate(
     cases_this_period = ifelse(
       location %in% c('NFLD.', 'SASK.')
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1953", format="%B %d %Y")
       & cases_this_period == '',
       'Not available',
       ifelse(
         location %in% c('NFLD.', 'SASK.') & cases_this_period == ''
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1953", format="%B %d %Y") ,
         '0',
         cases_this_period)))

   # AB starts reporting in 1936
   %>% mutate(
     cases_this_period = ifelse(
       location == 'ALTA.'
   ...
   # PEI is reporting from 1936-48, 1953 onward
   %>% mutate(
     cases_this_period = ifelse(
       # not available before 1936
       (location == 'P.E.I'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1936", format="%B %d %Y")
       & cases_this_period == '')
       # not available from 1949-53
       | (location == 'P.E.I'
          & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1953", format="%B %d %Y")
   ...
   # Quebec is not reporting between 1949-1953, otherwise they are reporting
   %>% mutate(
     cases_this_period = ifelse(
       location == 'QUE.'
       & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1949", format="%B %d %Y")
       ...
       & cases_this_period == '',
       'Not available',
       ifelse(location == 'QUE.' & cases_this_period == '' ,
              '0',
              cases_this_period)))

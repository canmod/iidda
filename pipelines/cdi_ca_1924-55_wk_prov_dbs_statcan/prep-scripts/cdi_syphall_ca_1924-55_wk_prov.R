...
tidy_dataset = "cdi_syphall_ca_1924-55_wk_prov"
digitization = "cdi_syphall_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Alberta, NB, PEI, and Saskatchewan start reporting in 1931
   %>% mutate(
     cases_this_period = ifelse(
       location %in% c('ALTA.', 'N.B.', 'P.E.I', 'P.E.I.', 'SASK.')
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1931", format="%B %d %Y")
       & cases_this_period == '',
       'Not available',
       ifelse(
         location %in% c('ALTA.', 'N.B.', 'P.E.I', 'P.E.I.', 'SASK.') & cases_this_period == ''
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1931", format="%B %d %Y") ,
         '0',
         cases_this_period)))

   # BC starts reporting in 1939
   %>% mutate(
     cases_this_period = ifelse(
       location == 'B.C.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1939", format="%B %d %Y")
       & cases_this_period == '',
       'Not available',
       ifelse(
   ...
   # Manitoba starts reporting in 1938
   %>% mutate(
     cases_this_period = ifelse(
       location == 'MAN.'
   ...
   %>% mutate(
     cases_this_period = ifelse(
       ...
       'Not available',
       ifelse(
         location == 'N.S.' & cases_this_period == ''
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1927", format="%B %d %Y") ,
         '0',
         cases_this_period)))

   # Quebec starts reporting in 1944
   %>% mutate(
     cases_this_period = ifelse(
       location == 'QUE.'
   ...
   # Ontario reporting monthly but not weekly until Feb 26, 1927
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk'
       & location == 'ONT.'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("February 26 1927", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '',0),
       'Not available',
       cases_this_period
     ))

   # Ontario not reporting monthly in 1924 after July 5
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & location == 'ONT.'
       & year(period_end_date) == 1924
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("July 5 1924", format="%B %d %Y")
       & cases_this_period %in% c('-', '—', '', 0),
       'Not available',
       cases_this_period
     ))

   # 1937: provinces which are reporting weekly have no monthly totals
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("January 1 1938", format="%B %d %Y")
   ...
   # Only Newfoundland is reporting monthly in 1952 , all other locations have weekly reports but not monthly totals
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & location != 'NFLD.'
   ...
   # 1944 first week is empty, assume it is not available (the following and previous weeks are reporting many cases)
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk'
       & as.Date(period_end_date, format="%B %d %Y") == as.Date("January 1 1944", format="%B %d %Y")
   ...
   # Override information from the phac portal and use common sense for some cases

   # 1944 Quebec switches from monthly to weekly mid-year
   |> mutate(
      cases_this_period = ifelse(
        time_scale == "wk"
        & location == "QUE."
        & as.Date(period_end_date) < as.Date("1944-10-07")
        & collection_year == "1944",
        "Not available",
        cases_this_period
   ))
   |> mutate(
   ...
   |> mutate(
      cases_this_period = ifelse(
          ...
          & location == "ONT."
          & collection_year == "1944"
          & as.Date(period_end_date) > as.Date("1944-05-06"),
          "Not available",
          cases_this_period
      )
   )

   # Otherwise assume that all provinces are reporting, so convert empty cells and dashes to 0

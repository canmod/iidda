...
tidy_dataset = "cdi_enceph_ca_1924-34_wk_prov"
digitization = "cdi_enceph_ca_1924-34_wk_prov"
...
missing_values = function(data){
  ...
  (filtered_data
   # In 1925, Canada and B.C. have monthly cases but not weekly from Oct 3 to Oct 31
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('B.C.', 'CANADA') & cases_this_period == ''
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("October 31 1925", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("October 3 1925", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))
    
    # In 1929, Quebec and Canada have monthly cases but not weekly from June 1 to June 29
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' & location %in% c('B.C.', 'CANADA') & cases_this_period == ''
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("June 29 1929", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("June 1 1929", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))

...
tidy_dataset = "cdi_diar_ca_1937-44_wk_prov"
digitization = "cdi_diar_ca_1937-44_wk_prov"
...
missing_values = function(data){
  ...
  filtered_data <- data %>%
    ...
    filtered_data <- select(filtered_data, -year)
  
  (filtered_data
    # Get rid of P.E.I. data for 1940 : there is 1 case but it is of enteritis
    %>% filter(!(location == 'P.E.I' &
                   as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1940", format="%B %d %Y") &
                   as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 28 1940", format="%B %d %Y")))

...
tidy_dataset = "cdi_uf_ca_1928-55_wk_prov"
digitization = "cdi_uf_ca_1928-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Alberta starts reporting in 1931
   %>% mutate(
   ...
   %>% mutate(
     cases_this_period = ifelse(
       ...
       'Not available',
       ifelse(
         location == 'B.C.' & cases_this_period == '' 
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1929", format="%B %d %Y") ,
         '0',
         cases_this_period)))
   
   # Manitoba reporting in 1931, 1934 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # 1931: empty = 0
       (location == 'MAN.'
        ...
        & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1931", format="%B %d %Y")
        & cases_this_period == '')
       # 1934 onwards: empty = 0
       | (location == 'MAN.'
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1934", format="%B %d %Y")
          & cases_this_period == '')
       , 0
       # Manitoba is otherwise not available
       , ifelse(
         location == 'MAN.' & cases_this_period == '',
         'Not available',
         cases_this_period)))
   
   # NB reporting in 1937-42, 1945, 1946, 1948, 1950, 1953
   %>% mutate(
     cases_this_period = ifelse(
       # 1937-42: empty = 0 
       (location == 'N.B.'
        & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1943", format="%B %d %Y")
        & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1937", format="%B %d %Y")
        & cases_this_period == '')
       # 1945, 46: empty = 0
       | (location == 'N.B.'
          & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1947", format="%B %d %Y")
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1945", format="%B %d %Y")
          & cases_this_period == '')
       # 1948: empty = 0
       | (location == 'N.B.'
          & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1949", format="%B %d %Y")
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1948", format="%B %d %Y")
          & cases_this_period == '')
       # 1950: empty = 0
       | (location == 'N.B.'
          & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1951", format="%B %d %Y")
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1950", format="%B %d %Y")
          & cases_this_period == '')
       # 1953: empty = 0
       | (location == 'N.B.'
          & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1954", format="%B %d %Y")
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1953", format="%B %d %Y")
          & cases_this_period == '')
       , 0
       # NB is otherwise not available
       , ifelse(
         location == 'N.B.' & cases_this_period == '',
         'Not available',
         cases_this_period)))
   ...
   # NS and Saskatchewan start reporting in 1930
   %>% mutate(
     cases_this_period = ifelse(
       location %in% c('N.S.', 'SASK.')
       & cases_this_period == ''
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1930", format="%B %d %Y") ,
       'Not available',
       ifelse(
         location %in% c('N.S.', 'SASK.') & cases_this_period == '' 
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1930", format="%B %d %Y") ,
         '0',
         cases_this_period)))
   
   # Ontario starts reporting in 1928
   %>% mutate(
     cases_this_period = ifelse(
       location == 'ONT.'
       & cases_this_period == ''
   ...
   # PEI reporting in 1936-48 , 1952 onwards
   %>% mutate(
     cases_this_period = ifelse(
       # 1936-48: empty = 0
       (location %in% c('P.E.I', 'P.E.I.')
        ...
        & cases_this_period == '')
       # 1952 onwards: empty = 0
       | (location %in% c('P.E.I', 'P.E.I.')
          & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1952", format="%B %d %Y")
          & cases_this_period == '')
       , 0
   ...
   # Quebec starts reporting in 1933
   %>% mutate(
     cases_this_period = ifelse(
       location == 'QUE.'
       & cases_this_period == ''
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1933", format="%B %d %Y") ,
       'Not available',
       ifelse(
         location == 'QUE.' & cases_this_period == '' 
         & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1933", format="%B %d %Y") ,
         '0',
         cases_this_period)))
   ...
   # timescales corresponding to the same period are empty
    ...
    # B.C. and Canada are reporting monthly but not weekly between Dec 7 1929 and Dec 28 1929
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' 
        & location %in% c('B.C.', 'CANADA') 
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 28 1929", format="%B %d %Y")
        & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("December 7 1929", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))
    
    # B.C. is reporting monthly but not weekly between may 3 1930 and Dec 31, 1932
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'wk' 
        & location == 'B.C.' 
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("May 3 1930", format="%B %d %Y") 
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y"),
        'Not available',
        cases_this_period
      ))
    
   # In 1952 , all locations have weekly reports but not monthly totals
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'mo'
       & cases_this_period %in% c('-', '—', '',0)

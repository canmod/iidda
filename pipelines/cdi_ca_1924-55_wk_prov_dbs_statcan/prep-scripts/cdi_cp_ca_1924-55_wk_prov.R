...
tidy_dataset = "cdi_cp_ca_1924-55_wk_prov"
digitization = "cdi_cp_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Newfoundland starts reporting on July 2, 1949
   %>% mutate(
     cases_this_period = ifelse(
       location == 'NFLD.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("July 2 1949", format="%B %d %Y") 
       & cases_this_period == '',
       'Not available',
       ifelse(
...
# causes warnings when applying max() to empty array , ignore 

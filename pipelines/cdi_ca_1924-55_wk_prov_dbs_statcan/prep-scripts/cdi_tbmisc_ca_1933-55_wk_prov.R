...
tidy_dataset = "cdi_tbmisc_ca_1933-55_wk_prov"
digitization = "cdi_tbmisc_ca_1933-55_wk_prov"
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  ...
  if(as.numeric(focal_sheet_info$year) < 1952){
    clean_wkmt = 
      (clean_weeks
       ...
       %>% ungroup()
       %>% mutate(period_end_date=ifelse(month!='TOT' & stat_name=='mo', month_end_date, period_end_date))
  ...
  case2 =  (clean_wkmt
            ...
            # if there are two unequal numbers in the wk and mt column , keep the larger number
...
missing_values = function(data){
  (data
   # -------------------------------------------------------------------------
   # Using information for TB from https://diseases.canada.ca/notifiable/charts?c=pl

...
tidy_dataset = "cdi_syphprim_ca_1944-55_wk_prov"
digitization = "cdi_syphprim_ca_1944-55_wk_prov"
...
missing_values = function(data){
  ...
  # timescales corresponding to the same period are empty
    ...
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' 
        & location %in% c('P.E.I', 'P.E.I.','N.S.','N.B.', 'ONT.', 'SASK.', 'MAN.', 'ALTA.', 'B.C.') 
        & cases_this_period %in% c('-', '—', '',0)
    ...
    # 1944: Ontario and Manitoba not reporting weekly until Sept 30, when large case counts start appearing.
    ...
    # 1944: Quebec is reporting weekly but not monthly between Oct 7 1944 - Dec 30 1944
    %>% mutate(
      cases_this_period = ifelse(
        time_scale == 'mo' & location == 'QUE.'
        & cases_this_period %in% c('-', '—', '',0)
        & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 30 1944", format="%B %d %Y")
        & as.Date(period_end_date, format="%B %d %Y") >= as.Date("October 7 1944", format="%B %d %Y"),
        'Not available',
        cases_this_period
      )
    )

...
tidy_dataset = "cdi_sst_ca_1924-55_wk_prov"
digitization = "cdi_sst_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Ontario and Canada are reporting monthly but not weekly until Dec 27 1924
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' 
       & location %in% c('ONT.', 'CANADA')
       & cases_this_period == '' 
       & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 27 1924", format="%B %d %Y"),
       'Not available',
       cases_this_period
     )) 
   
   # B.C. and Canada weekly not available between Nov 2 1929 and Dec 31 1932
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' 
       ...
       & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("November 2 1929", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))
   
   # Man and Canada weekly not available in September 1929
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' 
       & location %in% c('MAN.', 'CANADA')
       & cases_this_period == ''
       & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("September 28 1929", format="%B %d %Y")
       & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("September 7 1929", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))
   
   # Man weekly not available in 1931, 1932
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' 
       & location == 'MAN.'
   ...
   # Convert all other empty cells and dashes to 0

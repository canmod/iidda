...
tidy_dataset = "cdi_gonall_ca_1924-55_wk_prov"
digitization = "cdi_gonall_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   |> filter(!iidda::is_empty(trimws(cases_this_period)))
  # -------------------------------------------------------------------------
  ...
  # Ontario reporting in 1924, 1927 onwards
  %>% mutate(
    cases_this_period = ifelse(
       # not available from 1925-27
      (location == 'ONT.'
       & as.Date(period_end_date, format ="%B %d %Y") < as.Date("January 1 1927", format="%B %d %Y")
       & as.Date(period_end_date, format ="%B %d %Y") >= as.Date("January 1 1925", format="%B %d %Y")
       & cases_this_period == ''),
      'Not available',
      # Ontario is otherwise available
      ifelse(

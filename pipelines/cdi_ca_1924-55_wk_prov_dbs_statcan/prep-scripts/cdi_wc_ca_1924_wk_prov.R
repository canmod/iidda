...
tidy_dataset = "cdi_wc_ca_1924_wk_prov"
digitization = "cdi_wc_ca_1924_wk_prov"
...
behead_sheet = function(focal_sheet, data) {
  (data
   ...
   %>% mutate(fixed_statname=ifelse(is.na(location), 'mt', 'wk'))
...
clean_sheet=function(beheaded_sheet, focal_sheet_info){
  ...
  if(as.numeric(focal_sheet_info$year) == 1955){
    clean_weeks = (clean_weeks
                   %>% filter(!(period_start_date == '1954-12-26' & period_end_date == '1955-01-02'))
    )
  }
  ...
  if(as.numeric(focal_sheet_info$year) < 1952){
    clean_wkmt =
      (clean_weeks
       ...
       %>% ungroup()
       %>% mutate(period_end_date=ifelse(month!='TOT' & stat_name=='mt', month_end_date, period_end_date))# important!

       %>% group_by(month)
       %>% mutate(month_start_date=as.character(min(as.Date(period_start_date), na.rm=TRUE)))
       %>% ungroup()
       %>% mutate(period_start_date=ifelse(stat_name=='mt', month_start_date, period_start_date))
    ...
    clean_wkmt =
      (clean_weeks
       %>% mutate(index = ifelse(stat_name == "mt", four_weekly_period$index(week_ending_date), ""))
       %>% mutate(period_start_date = ifelse(stat_name == "mt" & month!= "TOT", as.character(four_weekly_period$start_date(index)), period_start_date))
       %>% mutate(period_end_date = ifelse(stat_name == "mt"& month!= "TOT", as.character(four_weekly_period$end_date(index)), period_end_date))
       %>% select(-index)
      )
  }
...
missing_values = function(data){
  (data
   ...
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' & location %in% c('ONT.', 'ALTA.')  & as.Date(period_end_date, format="%B %d %Y") <= as.Date("June 28 1924", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))
   ...
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' & location %in% c('B.C.', 'QUE.', 'MAN.') & as.Date(period_end_date, format ="%B %d %Y") <= as.Date("December 31 1932", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))

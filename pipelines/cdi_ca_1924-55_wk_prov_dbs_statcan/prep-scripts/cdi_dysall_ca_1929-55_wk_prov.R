...
tidy_dataset = "cdi_dysall_ca_1929-55_wk_prov"
digitization = "cdi_dysall_ca_1929-55_wk_prov"
...
behead_sheet = function(focal_sheet, data) {
  # In 1952, there are weekly case counts in the column that usually has the monthly case counts
  if(focal_sheet == '1952'){
    (data
     %>% filter(sheet == '1952', between(row, 3, 59), between(col, 2, 24))
     %>% select(row, col, data_type, numeric, character)
     %>% behead('N', location)
     %>% behead('N', stat_name)
     %>% behead('W', week_ending)
     %>% mutate(stat_name = ifelse(stat_name == 'mt', 'mo', stat_name))
     %>% mutate(stat_name=ifelse(is.na(location), 'wk', 'mo'))

     # 1952 NFLD has the normal ordering of wk and mt --> switch them
     %>% mutate(stat_name = ifelse(col == 4, 'mo', stat_name))
     %>% mutate(stat_name = ifelse(col == 3, 'wk', stat_name))
...
clean_sheet = function(beheaded_sheet, focal_sheet_info){
  ...
  if(as.numeric(focal_sheet_info$year) < 1952){
    clean_wkmt =
      (clean_weeks
       ...
       %>% ungroup()
       %>% mutate(period_end_date=ifelse(month!='TOT' & stat_name=='mo', month_end_date, period_end_date)) # important! why?
  ...
  case2 =  (clean_wkmt
            ...
            # if there are still duplicate cases, keep only the 'mo' column ones
...
missing_values = function(data){
  ...
  # in this file, there are many cases where there is no monthly/weekly data available
  # for an entire year, but there is a yearly total = '-'.
  # This code removes the weekly/monthly empty cells in such cases.
  filtered_wkmt <- (data
    %>% filter(time_scale %in% c('wk', 'mo'))
  ...
  filtered_data = rbind(filtered_wkmt, filter(data, time_scale == 'yr' & cases_this_period != ''))

  (filtered_data

  # BC has has no weekly data from 1929-33, but has monthly data on each sheet --> convert empty weekly cells up until then to 'Not available'
   %>% mutate(
  ...
  # From 1947 - 52 , locations with weekly cases do not have reported monthly cases --> for these locations, monthly case cells == 'Not available'
  # 1947: all but NFLD , NB, and PEI have no monthly totals (but have weekly cases)
  %>% mutate(
    cases_this_period = ifelse(
      time_scale == 'mo' & location != 'NFLD.' & location != 'N.B.'
      & location != 'P.E.I.' & location != 'ALTA.' & location != 'SASK.'
  ...
  # 1948,49: all but NFLD, NS , NB, and PEI have no monthly totals (but have weekly cases)
  %>% mutate(
    cases_this_period = ifelse(
      time_scale == 'mo' & location != 'NFLD.' & location != 'N.B.' & location != 'P.E.I.' & location != 'N.S.'
      & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 3 1948", format="%B %d %Y")
      & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1949", format="%B %d %Y")
      & cases_this_period == '',
      'Not available',
      cases_this_period
    ))

  # 1950: all but NFLD , PEI, and NB have no monthly totals (but have weekly cases)
  %>% mutate(
    cases_this_period = ifelse(
      time_scale == 'mo' & location != 'NFLD.' & location != 'N.B.' & location != 'P.E.I.'
      & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 3 1948", format="%B %d %Y")
      & as.Date(period_end_date, format="%B %d %Y") <= as.Date("December 31 1949", format="%B %d %Y")
      & cases_this_period == '',
      'Not available',
  ...
  # 1951: all but PEI and NS have no monthly totals (but have weekly cases)
  %>% mutate(
    cases_this_period = ifelse(
      time_scale == 'mo' & location != 'N.S.' & location != 'P.E.I.'
      & as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 6 1951", format="%B %d %Y")
  ...
  # 1952: all but NFLD have no monthly totals (but have weekly cases)
  %>% mutate(
    cases_this_period = ifelse(
      time_scale == 'mo' & location != 'NFLD.'
  ...
  # Convert all empty cells to 0 ; no evidence
  # that they are instead 'not available'
  ...
  # week ending jan 1st in 1955 is actually
  # reported at the end of the sheet for 1954

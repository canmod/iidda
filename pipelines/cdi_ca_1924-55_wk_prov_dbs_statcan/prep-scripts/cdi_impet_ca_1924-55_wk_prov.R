...
tidy_dataset = "cdi_impet_ca_1924-55_wk_prov"
digitization = "cdi_impet_ca_1924-55_wk_prov"
...
missing_values = function(data){
  (data
   ...
   # Quebec has a case count for the month of august 1929 but no weekly reports
   %>% mutate(
     cases_this_period = ifelse(
       time_scale == 'wk' & location == 'QUE.'  & as.Date(period_end_date, format="%B %d %Y") <= as.Date("August 31 1929", format="%B %d %Y")
       & as.Date(period_end_date, format="%B %d %Y") >= as.Date("August 3 1929", format="%B %d %Y"),
       'Not available',
       cases_this_period
     ))
   
   # Before 1949, all other empty cells = 0. After 1949, '-' means 0, and empty cells are taken to be 'Not available'
   %>% mutate(cases_this_period = ifelse(
     as.Date(period_end_date, format="%B %d %Y") < as.Date("January 1 1949", format="%B %d %Y") & cases_this_period == "", 
     0, 
     ifelse(as.Date(period_end_date, format="%B %d %Y") >= as.Date("January 1 1949", format="%B %d %Y") & cases_this_period == "", 
            'Not available', 
            cases_this_period)
   ))

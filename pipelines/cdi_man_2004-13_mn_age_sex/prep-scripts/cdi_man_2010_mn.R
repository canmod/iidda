...
tidy_dataset = "cdi_man_2010_mn"
digitization = "cdi_man_2010_mn"
...
clean_sheets = function(beheaded_sheet){
  (beheaded_sheet
   ...
   |> mutate(cases_statistic = case_when(
     stat_name == 2009 & col < 5 ~ 'cases_this_period',
     stat_name == 2009 & col >= 5 ~ 'cases_cum_prev_year',
     stat_name == 2010 & col >= 5 ~ 'cases_cum_report_year',
...
calculate_period_dates = function(focal_sheet, data, last_start_date = NULL, last_end_date = NULL) {
  ...
  date_string = grep('2010$', period$character, value = TRUE)

...
tidy_dataset = "cdi_man_2013_mn"
digitization = "cdi_man_2013_mn"
...
all_disease_names = function(data) {
  (data
    ...
    |> grep(pattern = "^Excludes", value = TRUE, invert = TRUE)
    |> grep(pattern = "^(DISEASE/INFECTION)", value = TRUE, invert = TRUE)
    |> grep(pattern = "^(Disease Name)", value = TRUE, invert = TRUE)
    |> grep(pattern = "^(Public Health Division)", value = TRUE, invert = TRUE)
...
clean_sheets = function(beheaded_sheet){
  (beheaded_sheet
   ...
   |> mutate(cases_statistic = case_when(
     stat_name == 2012 & col < 5 ~ 'cases_this_period',
     stat_name == 2012 & col >= 5 ~ 'cases_cum_prev_year',
     stat_name == 2013 & col >= 5 ~ 'cases_cum_report_year',
...
calculate_period_dates = function(focal_sheet, data, last_start_date = NULL, last_end_date = NULL) {
  ...
  date_string = grep('2013$', period$character, value = TRUE)
...
add_periods = function(sheets, data, cleaned_sheets) {
  ...
  tidy_data = (cleaned_sheets
               ...
               %>% mutate(historical_disease = ifelse(
                 historical_disease == 'MENINGOCOCCAL',
                 'MENINGOCOCCAL CSF',
                 historical_disease
               ))
...
# page 23 and 24 will be included in 2014 as they have a different format
sheets = sheets[!sheets %in% c('Page 23', 'Page 24')]

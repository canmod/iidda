...
tidy_dataset = "cdi_man_2007_mn"
digitization = "cdi_man_2007_mn"
...
clean_sheets = function(beheaded_sheet){
  (beheaded_sheet
   ...
   |> mutate(cases_statistic = case_when(
     stat_name == 2006 & col < 5 ~ 'cases_this_period',
     stat_name == 2006 & col >= 5 ~ 'cases_cum_prev_year',
     stat_name == 2007 & col >= 5 ~ 'cases_cum_report_year',
...
calculate_period_dates = function(focal_sheet, data, last_start_date = NULL, last_end_date = NULL) {
  ...
  date_string = grep('2007$', period$character, value = TRUE)
...
add_periods = function(sheets, data, cleaned_sheets) {
  ...
  tidy_data = (cleaned_sheets
               ...
               %>% mutate(historical_disease = ifelse(
                 historical_disease == 'MENIGOCOCCAL',
                 'MENINGOCOCCAL CSF',
                 historical_disease
               ))
